...
This portfolio was generated by ShellMentor v{APP_VERSION}.
...
def validate_challenge_output(user_output: str, expected_pattern: str,
    ...
    if validation_type == "pattern_match":
        ...
        pattern_lower = expected_pattern.lower() if expected_pattern else ""

        # Require multiple lines for "list" or "lines" patterns
        if "list" in pattern_lower or "lines" in pattern_lower:
            if len(non_empty) < 2:
                return False, f"Expected multiple lines of output, got {len(non_empty)}."

        # Check for numerical counts
        ...
        if "status" in pattern_lower or "http" in pattern_lower:
            if not re.search(r'\b[1-5]\d{2}\b', output):
                return False, "Expected HTTP status codes in the output."

        # Check for dates or timestamps
        if "date" in pattern_lower or "time" in pattern_lower or "log" in pattern_lower:
            if not re.search(r'\d{4}[-/]\d{2}[-/]\d{2}|\d{2}:\d{2}:\d{2}', output):
                return False, "Expected timestamps or dates in the output."

        # Check for sorted output (descending numeric)
        if "sorted" in pattern_lower or "top" in pattern_lower or "frequency" in pattern_lower:
            numbers = re.findall(r'^\s*(\d+)', output, re.MULTILINE)
            if len(numbers) >= 2:
